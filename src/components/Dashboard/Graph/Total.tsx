...
export default function Total() {
  ...
  function getSpent(month) {
    if (currentFinancialEvent) {
      ...
      const yearFlter = spentFilter.filter((item) => item.year === year);
  ...
  function getDonation(month) {
    if (currentFinancialEvent) {
      ...
      const yearFlter = donationFilter.filter((item) => item.year === year);

      const monthFlter = yearFlter.filter((item) => item.month === month);

      const donationInfo = monthFlter.reduce(function (
        accumulator,
        currentValue
      ) {
        return accumulator + JSON.parse(currentValue.value);
  ...
  const data = {
    ...
    datasets: [
      ...
      },
      {
        label: `Cash out - ${year}`,
        data: [
          getSpent(1) + getDonation(1),
          getSpent(2) + getDonation(2),
          getSpent(3) + getDonation(3),
          getSpent(4) + getDonation(4),
          getSpent(5) + getDonation(5),
          getSpent(6) + getDonation(6),
          getSpent(7) + getDonation(7),
          getSpent(8) + getDonation(8),
          getSpent(9) + getDonation(9),
          getSpent(10) + getDonation(10),
          getSpent(11) + getDonation(11),
          getSpent(12) + getDonation(12),
        ],
        fill: false,
        borderColor: "#da2c20",
      },
  ...
  function animateBySituation() {
    ...
    return (
      <div>
        ...
        <Lottie options={currentOptions} height={200} width={200} />
      </div>
    );
  }
